...
static int key_custom_read(void *data, char **key, cmph_uint32 *keylen) {

	cmph_vector_t *cmph_vector = (cmph_vector_t *) data;
	row **rows = (row **) cmph_vector->vector;
	*keylen = rows[cmph_vector->position]->m * sizeof(row_block);
	*key = (char *) malloc(*keylen);
	memcpy(*key, rows[cmph_vector->position]->blocks, *keylen);
	cmph_vector->position = cmph_vector->position + 1;
	return (int) (*keylen);
}
...
cmph_io_adapter_t *cmph_io_custom_vector_adapter(row ** vector, cmph_uint32 nkeys) {
	cmph_io_adapter_t * key_source = cmph_io_vector_new(vector, nkeys);
	key_source->read = key_custom_read;
	key_source->dispose = key_vector_dispose;
	key_source->rewind = key_vector_rewind;
	return key_source;
}
...
cmph_t *cmph_load(FILE *f) {
	...
	mphf = __cmph_load(f);
	if (mphf == NULL) return NULL;DEBUGP("Loading mphf algorithm dependent parts\n");
